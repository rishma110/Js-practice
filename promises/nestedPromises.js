...
const aggregatedData = (resolve, reject) => {
  ...
  for (let i = 0; i < datacenters.length; i++) {
    ...
    for (let j = 0; j < applicationTypes.length; j++) {
      arr.push(
        getApplicationProtectionStats(
          datacenters[i].url,
          applicationTypes[j]
        ).then((obj) => {
          return {
            name: datacenters[i].name,
            protectedApps: obj.protectedApps,
            unprotectedApps: obj.unprotectedApps,
          };
        })
      );
    }
  ...
  let p = Promise.all(arr).then((obj) => {
    obj.forEach((val) => {
      stats[val.name].protectedApps =
        stats[val.name].protectedApps + val.protectedApps;
      stats[val.name].unprotectedApps =
        stats[val.name].unprotectedApps + val.unprotectedApps;
    });
    resolve(stats);
  });
  return p;
...
let aggregatedData = function () {
  return new Promise(function (resolve, reject) {
    ...
    Promise.all(arr).then(function (obj) {
      obj.forEach((val) => {
        stats[val.name].protectedApps += val.protectedApps;
        stats[val.name].unprotectedApps += val.unprotectedApps;
      });
...
const actualStats = aggregatedData().then((val) => {
  console.log(val);
});
